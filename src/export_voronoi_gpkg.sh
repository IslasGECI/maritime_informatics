...
OUTPUT="${1:-data/processed/maritime_context.gpkg}"
PG_CONN="PG:host=postgis user=postgres dbname=postgres"

# Create parent directory
mkdir --parents "$(dirname "$OUTPUT")"

# Remove existing file to prevent layer accumulation across runs
rm -f "$OUTPUT"
...
ogr2ogr -f GPKG -t_srs EPSG:4326 "$TMP_GPKG" "$PG_CONN" context_data.europe_coastline_polygon
ogr2ogr -f GPKG \
    -update \
    -append \
    -clipsrc -6 47 -1 49 \
